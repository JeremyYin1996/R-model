...
for (i in 1:100){
  ...
  web <- read_html(str_c("http://nj.lianjia.com/ershoufang/pg", i), encoding = "UTF-8")
...
ggplot() + 
  geom_point(data = house1, aes(x =data1$house_name, y = data1$house_unitprice, col = Species)) +
  geom_polygon(data = hulls, aes(fill=Species), alpha = 0.5) 
